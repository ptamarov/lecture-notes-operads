...
It is useful to observe that if we take $M = \{\star \}$ the trivial
monoid, the resulting 
\begin{enumerate}

\item Words operads.
\item Associative operad.
\end{enumerate}

\subsection{Path sequences and leaf permutations}

Part 2.
\begin{enumerate}
\item Monomial orders on shuffle operads.
\item Path-permutation extension.
\item Graded path-permutation 
	lexicographic order.
\item Examples. 
\end{enumerate}
