...
which made it possible to produce these lectures notes
...
the seventies, with the objective of streamlining
...
and to quadratic operads in particular, and define what
it means for such operads to be Koszul. 
...
\begin{algorithmic}[1]
\Procedure{LongDiv}{$f$,$V$}
\If {$f=0$} \Return $f$
	 \Else
	\State $V \gets \textsc{LinSelfRed}(V)$
	\While {$\{ v\in V : \lead{v} \text{ divides } \lead{f}\}\neq \varnothing$}
	\State $V'\gets \{ v\in V : \lead{v} \text{ divides } \lead{f}\}$
	\State $s\gets \textsc{Last}(V')$   
	\State $f'\gets \textsc{Reduce}(f,s)$ 
	\State $f\gets \textsc{LongDiv}(f',V)$  
		\EndWhile
	\State \Return $\lead{f}+ \textsc{LongDiv}(f-\lead{f},V)$
	\EndIf
\EndProcedure
\end{algorithmic}
\end{adjustwidth}
...
\Procedure{SelfRed}{$V$}
	\State $V \gets \textsc{LinSelfRed}(V)$
\If {$V$ is self reduced} \Return $V$
\Else 
\State $v\gets \textsc{Last}(V')$
\State $V'\gets \textsc{SelfRed}(V\smallsetminus v)$
\State $v'\gets \textsc{LongDiv}(V',v)$
\EndIf
\State \Return $\textsc{SelfRed}(V'\cup \{v'\})$
\EndProcedure
\end{algorithmic}
\end{adjustwidth}
...
Finally, suppose that for some $f$, two different choices of order of reductions yield two different outputs. Then, then there exist a coset $f + I$ contains two different elements that are reduced with respect to $\mathcal G$, hence reduced monomials are linearly dependent, a contradiction.
\end{proof}
...
Prove Proposition~\ref{lemma:self-reduction}.
\end{question}
\newpage
