...
\missingfigure{Picture of the map $\gamma'$}
...
\missingfigure{Picture of a bar tree monomial and its boundary}
...
\missingfigure{Diagram with syzygy grading}
...
\subsection{The Koszul dual homology cooperad}
...
that the dual of the cooperad  $\sus^c\otimes \PP^\emph{\antishriek}$
is isomorphic to $\PP^!$.
\end{question} 

\begin{question}\label{ex:dgbar} Suppose that $\PP$ is a dg
operad, and let us write $\partial_1$ for the differential of its
bar construction (considered as a non-dg operad). 
Show that in this case the differential $d_\PP" \PP
\longrightarrow \PP$ also induces a differential $\partial_2$
on $\FF_{s\PP}$, and that $\partial_1\partial_2 + \partial_2\partial_1=0$
is equivalent to $d)\PP$ being a derivation for $\gamma_\PP$.
Conclude that $\partial = \partial_1 + \partial_2$ is a differential
on $\FF_{s\PP}$. We call the resulting cooperad the bar construction of
...
\hacer{Organize the exercises logically.}
 
\afterpage{\blankpage}
\newpage
\section{Koszul duality III: Koszul complexes}

\textbf{Goals.} Give the definition of the
Koszul complexes associated to a quadratic operad,
and give a second definition of Koszulness.

\subsection{Twisting morphisms and Koszul complexes}
\hacer{Escribir esta secci\'on.}

\subsection{The bar-cobar adjunction}
...
two functors for an \emph{adjoint pair} 
\[
\B{-} : \mathsf{Op} \adjoint
   \mathsf{Coop}^c  : \Omega(-)
\]
...
\emph{conilpotent} augmented dg cooperads. 
Let us make this statement precise, and in the way discover
a third construction that will be crucial for us:
...
$\CC$, there exist a natural bijection
\[
\hom_{ \mathsf{Op}}(\CC,\B{\PP}) \longrightarrow
 \hom_{\mathsf{Coop}^c }(\Omega(\CC),\PP).
\]
\end{theorem}
...
\begin{definition}
We write $\Tw(\CC,\PP)$ for the set of morphisms above,
and call it the set of \emph{twisting morphisms} from
$\CC$ to $\PP$.
\end{definition}

\hacer{The Rosetta Stone.}
