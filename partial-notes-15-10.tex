...
\hacer{Write introduction to Koszul duality.}
...
$\XX$ in each of the following cases:
\begin{enumerate}
\item $\XX(2)=\mathbb C^2$ is 
	the regular representation of $S_2$.
\item $\XX(2)=\mathbb C^-$ is the sign representation of $S_2$.
\item $\XX(2)=\mathbb C$ is the trivial representation of $S_2$. 
\end{enumerate} 
...
\begin{question} In the trivial and sign case above,
compute a monomial tree basis, and thus the dimension, of $\mathcal{F}_\XX(4)$.
\end{question}
...
\begin{question}
Show that the Koszul dual of the operad controlling
...
\begin{question}
Suppose that $\mathcal P$ is binary quadratic
generated by an operation with no symmetries
...
\[
  x_1(x_2x_3) = 
 	...
 	\]
Show that its Koszul dual operad is presented by
the relation
\[
  (x_1x_2)x_3 = 
   	\sum_{\sigma \in S_3} 
...
\begin{proof}
This is in \textbf{Exercise Sheet \#5}. The
...
to repeat the computation of a basis
...
in situations like this it is crucial to observe
that $\underline{\XX}$ is obtained through a disjoint of the components
...
\begin{proof}
This is in \textbf{Exercise Sheet \#6}.
\end{proof}
...
\begin{proof}
\hacer{Write proof, look at Hoffbeck.}
\end{proof}
...
\missingfigure[figwidth = 6 cm]{Example of divisors.}
...
\end{proof}
\missingfigure[figwidth = 6 cm]{Divisors example}
...
\missingfigure[figwidth = 6 cm]{Insertion examples}

\begin{lemma}
Let $\mathscr{V}$ be a subset of the free shuffle operad on $\XX$.
Then the ideal generated by $\mathscr{V}$ is explicitly obtained
as the linear span of all insertions $\repl{T'}{T}{f}$
as $T,T'$ range through pairs $(T',T)$ with $T'$ a divisor
of $T$ and $f\in \mathscr V(\ell')$.
\end{lemma}

\begin{proof}
By construction, $(\mathscr V)$ is the linear span of all possible
shuffle compositions where at least one summand is contained in
$\mathscr V$. Since shuffle compositions are multilinear, we can
assume that all terms appearing in such shuffle compositions (except,
possibly, for that in $\mathscr V$) are tree monomials, in which case
the resulting shuffle composition coincides with an insertion
operation.
\end{proof}

\subsection{Leading terms and reducibility}
...
\missingfigure[figwidth = 6 cm]{Leading term example}
...
Let $\mathscr{V}$ be a subset of the free shuffle operad on $\XX$. A
tree monomial $T$ is reduced with respect to $\mathscr{V}$ if it is
not divisible by any of the leading terms of polynomials appearing in it.
A polynomial is reduced with respect to $\mathscr V$ if it is a linear
combination of tree monomials that are reduced. We say $\mathscr V$
is self-reduced if each $v\in \mathscr V$ is reduced with respect to
$\mathscr V\smallsetminus v$, and that is is linearly self-reduced
...
Its input is a polynomial $f$ and a finite set $\mathscr V$, both
in $\FF_\XX$, and its
output is a reduced element $\overline{f}$ with respect to $\mathscr
V$ such that $f-\overline{f} \in (\mathscr V)$ and $\lead{\overline{f}}
\preceq \lead{f}$.

\begin{tenumerate}
\item If $f=0$, return $f$.
\item Replace $\mathscr V$ by its linear self-reduction. 
\item While $\mathscr V' = \{ v\in \mathscr V : \lead{v} \text{ divides } \lead{f}\}$ is non-empty, pick $s\in \mathscr V'$ with the least
leading monomial, and return the result of long division of
$f'  = r_s(f)$ by $\mathscr V$.
\item Else, $\lead{f}$ is reduced, so let $f'$ be the result of long
division of $f-\lead{f}$ by $\mathscr V$, and return $\lead{f} + f'$.
\end{tenumerate}
...
division to $r_g(f) = f'$ for some $g\in\mathscr V$, and
...
$\mathscr V$, so the coset of $f$ is not modified,
which concludes our proof.
\end{proof}

\begin{proposition}
Suppose that $\mathscr I$ is an ideal in the free shuffle
operad generated by $\XX$. Then those shuffle monomials that
are reduced with respect to $\mathscr I$ form a basis
for the quotient operad $\FF_\XX / \mathscr I$.  
\end{proposition}
...
respect to $I$, and that $f\in \mathscr I$. Then we see that
$\lead{f} \in \lead{\mathscr I}$. But this can only happen if
...
that may appear in the elements of $\mathscr I$,
and Gr\"obner bases are designed to regain this control. 
 
 \bigskip
 
 \hacer{Mention the algorithm of self-reduction.}
 
\subsection{Existence and uniqueness}

\begin{lemma}
Let $\mathscr I$ be an ideal of $\FF_\XX$. The subspace 
\[
\lead{\mathscr I} = \langle T \in \FF_\XX : \text{ $T = 
	\lead{f}$ for some  $f\in\mathscr I$} \rangle.
\]
spanned by leading terms of elements of $\mathscr I$ is
again an ideal of $\FF_\XX$.
\end{lemma}

\begin{proof}
By construction $\lead{\mathscr I}$ is a subspace of $\FF_\XX$, so 
...
monomials if at least one term is already in $\lead{\mathscr I}$.
To do this, note that if
$T$ is the leading term of some $f\in\mathscr I$, then by
Proposition~\ref{prop:repllead}, for any tree monomials
$T_0,\ldots,T_{i-1},T_{i+1},\ldots,T_n$, the leading term
of the composition 
\[ \gamma_\pi(T_0;T_1,\ldots,T_{i-1},f,T_{i+1},\ldots,T_n)\in \mathscr I\]
is precisely $\gamma_\pi(T_0;T_1,\ldots,T_{i-1},T,T_{i+1},\ldots,T_n)$,
which proves this belongs to $\lead{\mathscr I}$.
\end{proof}

\begin{definition}
Let $\mathscr I$ be an ideal of $\FF_\XX$. We say that a subset
$\mathscr G$ of $\mathscr I$ is a \emph{Gr\"obner basis of $\mathscr I$}
(with respect to our fixed monomial order) if the set of leading monomials
of $\mathscr G$ generate the ideal of leading terms of $\mathscr I$.
A Gr\"obner basis which is self-reduced is called reduced.
\end{definition}

\begin{lemma}
Let $\mathscr I$ be an ideal and let $\mathscr G$ be a 
Gr\"obner basis of $\mathscr I$. Then $\mathscr G$ 
generates $\mathscr I$.  
\end{lemma}

\begin{proof}
Suppose that there is some $f\in \mathscr I$ that is not
generated by $\mathscr G$, and let us pick one with the least 
possible leading term. Since $\mathscr G$ generates the
ideal of leading terms of $\mathscr I$, we can reduce
the leading term of $f$ with respect to $\mathscr G$
without modifying its coset in $\mathscr I$,
and obtain an element that is generated by $\mathscr G$.
But then $f$ itself is generated by $\mathscr G$,
which is a contradiction.
\end{proof}


\begin{proposition}
A set $\mathscr G$ is a Gr\"obner basis if the cosets of monomials
...
$\mathscr G$ is independent of the choices or the order in
...
to $\mathscr G$ form a basis of the quotient operad precisely
when every coset of $\mathscr I$ contains a unique element
that is reduced with respect to $\mathscr G$. 

By the long division algorithm, it follows that every coset
contains at least one element that is reduced with respect to $\mathscr G$,
so it suffices we prove that this element is unique if and only if
$\mathscr G$ is a Gr\"obner basis.

Thus, first suppose that $\mathscr G$ is a Gr\"obner basis, but that
there exist two $\mathscr G$-reduced monomials that have the same
coset modulo $\mathscr I$. This means there exists a $\mathscr G$-reduced 
polynomial in $\mathscr I$, which means that its leading term is
$\mathscr G$-reduced, which is impossible.

Conversely, Suppose that $\mathscr G$ is not a Gröbner basis. It follows
that there is an element $f\in\mathscr I$ which is reduced with respect to 
$\mathscr G$. If we let $\overline{f}$ be the result of the long division of
$f$ by $\mathscr G$, we see we obtain a non-trivial linear combination of reduced 
monomials belonging to $\mathscr I$, so that there is not a unique
reduced representative for the zero coset.

Finally, suppose that for some $f$, two different choices of order of reductions yield two different outputs. Then, then there exist a coset $f + I$ contains two different elements that are reduced with respect to $\mathscr G$, hence reduced monomials are linearly dependent, a contradiction.
\end{proof}
