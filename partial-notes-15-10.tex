...
Since we will use them often, let us define the infinitesimal
composition  between two symmetric sequences $\XX$ and $\YY$,
which we write $\XX\circ_{(1)} \YY$, to be the subfunctor
of $\XX\circ(\kk\oplus \YY)$ that is linear in $\YY$. In other
words, we consider corollas whose root vertex is decorated by
$\XX$, and all whose leaves are ``empty'', except for one
which is decorated by an element of $\XX$.
...
\begin{question}
Show that the bijection described in Theorem~\ref{thm:adjunction} is
natural. 
\end{question}
\afterpage{\blankpage}
\newpage
